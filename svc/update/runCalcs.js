...
async function runCalcs(core) {
  const { content_url, content_minutes, content_type, publication_date } = core;
  const algo_input = {
  ...
  const objectiveMeasures = ["recency", "brevity"];
  const bonuses = {};
  const factors = {};
  const weighted = {};
  const multipliers = {};
  ...
  Object.keys(processed_algo_input).forEach((key) => {
    ...
    multipliers[key] = +(
      Math.pow(factors[key], weight) / Math.pow(floor, weight)
    ).toFixed(3);
  ...
  const item = { updated_at, calcs, scores, algo_input, core };
...
async function processAlgoInput(content_url, { brevity, recency }) {
  ...
  processed.recency = moment()
    .tz("America/Chicago")
    .diff(moment(recency), "days");
...
function getBonus(type, value) {
  ...
  const inverseTypes = ["brevity", "recency"];
...
async function countReferences(content_url) {
  ...
  for (referenceType in Reference) {
    try {
      references[referenceType] = await Reference[
        referenceType
      ].countDocuments({ content_url });
    } catch (e) {
      console.log({ e });
    }
  }
